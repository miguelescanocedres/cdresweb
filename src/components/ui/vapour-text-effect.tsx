...
export default function VaporizeTextCycle({
  ...
  const fontConfig = useMemo(() => {
    ...
    return {
      ...
      font: `${font.fontWeight ?? 900} ${fontSize * globalDpr}px ${font.fontFamily}`,
    }
...
const renderCanvas = ({
  ...
  const fontSize = parseInt(framerProps.font?.fontSize?.replace("px", "") || "120")
  const font = `${framerProps.font?.fontWeight ?? 900} ${fontSize * globalDpr}px ${framerProps.font?.fontFamily ?? "sans-serif"}`
  const color = parseColor(framerProps.color ?? "rgb(0, 102, 255)")

  let textX
  const textY = canvas.height / 2
  const currentText = framerProps.texts[currentTextIndex] || "CDRES"
